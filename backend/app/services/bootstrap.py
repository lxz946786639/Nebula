...
async def bootstrap_defaults(session: AsyncSession) -> None:
    ...
    defaults = {
        ...
        "smart_proxy_port_start": (str(settings.SMART_PROXY_PORT_START), False, "First port used by automatic smart proxy allocation"),
        "smart_proxy_port_end": (str(settings.SMART_PROXY_PORT_END), False, "Last port used by automatic smart proxy allocation"),
