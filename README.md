...
启用流量保护后，刷新流量时如果发现订阅从可用变为到期/耗尽，或从到期/耗尽恢复，Nebula 会对受影响且被智能代理引用的节点做一次 TCP 可用性检测。到期/耗尽且节点不可用时会自动重新应用 Mihomo runtime 剔除该订阅节点；恢复且节点可用时会自动重新应用 runtime 加回候选节点。
