...
- 蚂蚁代理：支持 Ant 账号登录或上传 `ant.db` 读取节点，并将蚂蚁节点接入智能代理统一调度。
- 流量保护：订阅到期或流量用完后，可自动避开不可用节点；订阅恢复后可自动重新纳入候选。
- Web 管理后台：提供订阅、节点、智能代理、配置、日志和实时状态页面。

## v1.1.2 更新要点

- 智能代理状态同步升级：Nebula 会按「智能代理全局配置 > 监控频率」从 Mihomo 回写运行状态和当前节点，避免 Mihomo 实际节点与页面展示不一致。
- 稳定优先策略增强：当前节点不可用且存在可用候选节点时，会自动切换并把新节点固化为主节点，原节点恢复后不会随意切回。
- 智能代理交互优化：运行状态、健康检测、诊断日志、节点切换历史和应用状态的中文提示更加清晰，并在当前节点与 Mihomo 运行态未同步时展示「待同步」提示。
- 节点来源筛选优化：订阅节点来源保留「所有节点 / 指定订阅 / 指定节点」，国家、标签和协议类型作为二级过滤条件，策略节点只从候选节点中选择。
- 代理流量统计增强：Mihomo 核心流量与蚂蚁代理适配器上下行流量支持持久化，重启后不再丢失累计数据。
- 蚂蚁代理与 Mihomo runtime 更稳：优化 Docker/Ubuntu 环境下的内部适配器地址解析、DNS 注入、测速和重启恢复逻辑。

## v1.1.1 更新要点

- 新增蚂蚁代理：支持 Ant 账号登录和上传 `ant.db` 两种方式，节点数据持久化保存到 Nebula，不依赖用户本机文件路径。
- 蚂蚁代理节点按「免费专线 / 付费专线」展示，账号登录支持手动刷新和后台定时刷新，`ant.db` 登录在重新上传时更新。
- 智能代理接入蚂蚁节点数据源，可按全部节点、指定节点、地区、免费/付费线路和协议类型筛选候选节点。
- 代理调度统一收敛到 Mihomo：蚂蚁节点通过内部 SOCKS 适配器接入 Mihomo runtime，对外仍由智能代理提供 HTTP、SOCKS5 或 Mixed 入口。
- UI 完成整体重构：侧边栏、顶部栏、登录页、列表页、表单、弹窗、移动端卡片和主要操作按钮统一视觉与交互风格。
- 移动端与 PWA 继续增强：内置 Web App Manifest、Service Worker、离线兜底页、安装按钮、iOS 主屏图标和前端更新提示。
...
6. 如需使用蚂蚁代理节点，进入「蚂蚁代理」完成账号登录或上传 `ant.db`，再到「智能代理」新增代理并选择数据来源「蚂蚁节点」。
...
「蚂蚁代理」页面用于把 Ant 客户端中的节点接入 Nebula：

- 登录方式：支持 Ant 账号登录，也支持上传 `ant.db` 文件解析节点。
- 数据持久化：登录或上传成功后，节点和登录来源会保存到 Nebula 数据库；经过蚂蚁节点内部适配器转发的上行/下行流量也会累计保存到数据库。
